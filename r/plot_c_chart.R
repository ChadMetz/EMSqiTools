#' plot_c_chart
#'
#' C Chart with annotations, tracking counts per unit over time.
#'
#' @param df Dataframe.
#' @param date_col Column name (string) for date/time.
#' @param id_col Unique identifier column (string).
#' @param count_condition Condition string (R expression) to filter events to count.
#' @param den_condition Optional filter for denominator (string, R expression).
#' @param time_unit Time unit for aggregation ('week', 'month', 'quarter').
#' @param name Chart title.
#' @param annotations Optional dataframe with columns Date, Label, Side, Shape, Y_Pos.
#'
#' @return A summary dataframe and displays the C chart.
#'
  ...
  if (!is.null(annotations)) {
    annotations <- annotations %>%
      mutate(
        ...
        Y_Pos = ifelse(is.na(Y_Pos), max(summary$UCL, na.rm = TRUE) + 0.5, Y_Pos),
